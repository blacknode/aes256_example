...
b64_encode(const unsigned char *src, size_t len)
{
  ...
  while (len--)
  {
    ...
    if (3 == i)
    {
      ...
      if (b64_buf_realloc(&encbuf, size + 4) == -1)
        return NULL;
  ...
  if (i > 0)
  {
    ...
    for (j = 0; (j < i + 1); ++j)
    {
      if (b64_buf_realloc(&encbuf, size + 1) == -1)
        return NULL;
...
b64_decode_ex(const char *src, size_t len, size_t *decsize)
{
  ...
  while (len--)
  {
    ...
    if (4 == i)
    {
      ...
      if (b64_buf_realloc(&decbuf, size + 3) == -1)
        return NULL;
      for (i = 0; i < 3; ++i)
      {
  ...
  if (i > 0)
  {
    ...
    if (b64_buf_realloc(&decbuf, size + (i - 1)) == -1)
      return NULL;
    for (j = 0; (j < i - 1); ++j)
    {
  ...
  // Make sure we have enough space to add '\0' character at end.
  if (b64_buf_realloc(&decbuf, size + 1) == -1)
    return NULL;
